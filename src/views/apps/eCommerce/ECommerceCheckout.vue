    ...
    <div id="ecommerce-checkout-demo">
        <form-wizard
            ...
            <tab-content title="Cart" icon="feather icon-shopping-cart" class="mb-5">
                ...
                <!-- IF CART HAVE ITEMS -->
                <div class="vx-row" v-if="cart.length" appear>

                    <!-- LEFT COL -->
                    <div class="vx-col lg:w-2/3 w-full relative">
                        <div class="items-list-view" v-for="(item, index) in cart" :key="item.id">
                ...
                <vx-card title="You don't have any items in your cart." v-else>
                    <vs-button @click="$router.push('/')">Browse Shop</vs-button>
                </vx-card>
...
export default {
    metaInfo: {
      ...
      meta: [
      { name: 'og:description', content: 'Checkout | Buy Now' }
    ]
    },
    mounted() {
        console.log(this.$route.params.cart);
        this.setCart(this.$route.params.cart);
    },
    data() {
        return {
            cart: [],
    ...
    methods: {
        setCart(cartItems) {
            let fire = this;
            fire.cart = cartItems;
            fire.cart.forEach(function(elm){
                fire.totalPrice += elm.price;
            });
        },
        // TAB 1
        ...
        wishListButtonClicked(item) {
            // Toggle in Wish List
            if (this.isInWishList(item.id)) this.$router.push('/apps/eCommerce/wish-list')
            else {
        ...
        updateItemQuantity(event, index) {
            const itemIndex = Math.abs(index + 1 - this.cart.length)
            this.cart[index].quantity = event;
            console.log("Event : " + this.cart[index].quantity)
